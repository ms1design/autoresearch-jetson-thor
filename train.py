...
class Block(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, block_mask):
        if self.use_gradient_checkpointing and self.training:
            ...
            def _inner_forward(x, ve, cos_sin, block_mask):
            ...
            return checkpoint(
                _inner_forward, x, ve, cos_sin, block_mask, use_reentrant=False
            )
        else:
            x = x + self.attn(norm(x), ve, cos_sin, block_mask)
            x = x + self.mlp(norm(x))
...
print(
    f"Unified memory cap: {UNIFIED_MEMORY_LIMIT_GB} GB "
